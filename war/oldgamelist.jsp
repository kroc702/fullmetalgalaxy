...
<% } else { 
	...
	if( !Auth.isUserLogged( request, response ) && (tab <= 1) ) {
		...
		// new or open games
		gameList = OldFmgDataStore.getPersistGameList();
		((com.googlecode.objectify.Query<PersistGame>)gameList).filter( "m_isOpen", true );
		gameCount = ((com.googlecode.objectify.Query<PersistGame>)gameList).count();
		...
		com.googlecode.objectify.Query<PersistGame> query = OldFmgDataStore.getPersistGameList();
