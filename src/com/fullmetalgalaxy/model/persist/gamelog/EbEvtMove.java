...
public class EbEvtMove extends AnEventPlay
{
  ...
  public void check(EbGame p_game) throws RpcFmpException
  {
    ...
    {
      throw new RpcFmpException( "vous ne pouvez pas d�placer votre astronef" );
    }
 ...
 p_game
    ...
    for( EbToken token : tokensOnWay )
    {
      ...
      {
        throw new RpcFmpException( "Vous devez d�placer votre pions sur une case libre" );
      }
    ...
    p_game.getBoardFireCover().disableFireCover( getToken(p_game) );
    ...
    EnuColor fireCoverColorNew = p_game.getOpponentFireCover( myRegistration.getColor(),
        getNewPosition() );
    p_game.getBoardFireCover().enableFireCover( getToken(p_game) );
