...
/*****************************************************************
* due to my modif, I add this security check
******************************************************************/
/* ok, it's not working :(
if( ($_GET['key'] != 'qs564op') 
	&& ($_POST['key'] != 'qs564op') 
	&& ($_REQUEST['key'] != 'qs564op') 
	&& (!strpos($_SERVER['QUERY_STRING'], "qs564op") ))
{
	redirect();
	//print_r( $_GET['key'] );
	//print_r( $_POST['key'] );
...
switch ( true ) {
   ...
   case ! empty($_GET['url']) && ( $toLoad = deproxifyURL($_GET['url'], true) ):
      break;
...
if ( $options['allowCookies'] ) {
   ...
   if ( $CONFIG['cookies_on_server'] ) {
      ...
      if ( $s = checkTmpDir($CONFIG['cookies_folder'], 'Deny from all') ) {
         ...
         $toSet[CURLOPT_COOKIEFILE] = $toSet[CURLOPT_COOKIEJAR] = $CONFIG['cookies_folder'] . "mycookie";//session_id();
